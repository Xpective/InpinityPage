/* =========================================================
   ...
   HARDENED GRID VERSION
   - Rarity back in grid
   - Claim info back in grid (lazy, throttled)
   ...
   - Reduced subgraph / onchain burst
   ========================================================= */
  ...
  const TOKEN_META_TTL_MS = 60_000;
  const TOKEN_RARITY_TTL_MS = 60_000;
  const V5_STATUS_TTL_MS = 45_000;
  const CLAIM_PREVIEW_TTL_MS = 12_000;
  const CLAIM_PREVIEW_CONCURRENCY = 2;
  const V5_CHECK_GAP_MS = 120;
  
  const tokenMetaCache = new Map();     // tokenId -> { expiresAt, value:{ revealed,row,col } }
  const tokenRarityCache = new Map();   // tokenId -> { expiresAt, value:number|null }
  const v5StatusCache = new Map();      // tokenId -> { expiresAt, value:boolean }
  const claimPreviewCache = new Map();  // key -> { expiresAt, value:{ ready,text,waitSec } }
  
  let selectedBlockClaimTicker = null;
  let latestSelectRequestId = 0;
  let latestGridRenderId = 0;
  ...
  async function getTokenMetaCached(tokenId) {
    const cached = getCached(tokenMetaCache, tokenId);
    ...
    try {
      const bd = await state.nftContract.blockData(tokenId);
    ...
    setCached(tokenMetaCache, tokenId, value, TOKEN_META_TTL_MS);
    return value;
  }
  
  async function getTokenRarityCached(tokenId, revealed) {
    if (!revealed) return null;
  
    const cached = getCached(tokenRarityCache, tokenId);
    if (cached !== null && cached !== undefined) return cached;
  
    try {
      const rarity = Number(await state.nftContract.calculateRarity(tokenId));
      setCached(tokenRarityCache, tokenId, rarity, TOKEN_RARITY_TTL_MS);
      return rarity;
    } catch {
      setCached(tokenRarityCache, tokenId, null, 10_000);
      return null;
    }
  }
  
  async function getV5StatusCached(tokenId) {
    const cached = getCached(v5StatusCache, tokenId);
    if (cached !== null) return cached;
  
    try {
      const active = await isTokenActiveOnV5(tokenId);
      setCached(v5StatusCache, tokenId, !!active, V5_STATUS_TTL_MS);
      return !!active;
    } catch {
      setCached(v5StatusCache, tokenId, false, 10_000);
      return false;
    }
  }
  
  function getClaimCacheKey(source, tokenId) {
    return `${source}:${tokenId}`;
  }
  
  async function getClaimPreviewCached(tokenId, { farmingActive, activeOnV5 }) {
    if (!farmingActive && !activeOnV5) {
      return {
        ready: false,
        text: "",
        waitSec: 0
    ...
    const source = farmingActive ? "v6" : "v5";
    const key = getClaimCacheKey(source, tokenId);
    const cached = getCached(claimPreviewCache, key);
    if (cached) return cached;
  
    try {
      let result = { ready: false, text: "", waitSec: 0 };
  
      if (farmingActive && state.farmingV6Contract) {
        ...
        result = waitSec <= 0
          ? {
              ready: true,
              text: `<div class="farm-duration" style="color:#5CFF95;">💰 Claim Ready</div>`,
              waitSec
            }
          : {
              ready: false,
              text: `<div class="farm-duration">💰 Claim: ${formatDuration(waitSec)}</div>`,
              waitSec
            };
      } else if (activeOnV5) {
      ...
      setCached(claimPreviewCache, key, result, CLAIM_PREVIEW_TTL_MS);
      return result;
    } catch {
      const fallback = { ready: false, text: "", waitSec: 0 };
      setCached(claimPreviewCache, key, fallback, 8_000);
      return fallback;
    }
  }
  ...
  async function updateGridClaimLinesLazy({
    renderId,
    tokens,
    v5Map,
    farmMap
  }) {
    await runWithConcurrency(tokens, CLAIM_PREVIEW_CONCURRENCY, async (token) => {
      if (renderId !== latestGridRenderId) return;
      ...
      if (!farmingActive && !activeOnV5) return;
  
      const claim = await getClaimPreviewCached(tokenId, {
        farmingActive,
        activeOnV5
      });
      ...
      const claimHost = document.querySelector(`.block-card[data-tokenid="${tokenId}"] .claim-line-host`);
      if (claimHost) {
        claimHost.innerHTML = claim.text || "";
      }
    });
  }
  ...
  export async function loadUserBlocks({ onRevealSelected, onRefreshBlockMarkings } = {}) {
    ...
    const renderId = ++latestGridRenderId;
  
    try {
      const [subgraphTokens, subgraphFarmsV6, subgraphProtections] = await Promise.all([
        loadMyTokensFromSubgraph(state.userAddress),
        loadMyFarmsV6FromSubgraph(state.userAddress),
        loadMercenaryTokenProtectionsV4()
      ]);
      ...
      const v5Map = new Map();
      for (const tokenId of state.userBlocks) {
        const active = await getV5StatusCached(tokenId);
        v5Map.set(String(tokenId), !!active);
      ...
      for (const token of subgraphTokens) {
        ...
        const meta = await getTokenMetaCached(tokenId);
        if (renderId !== latestGridRenderId) return;
  
        const revealed = !!(token.revealed || meta?.revealed);
        ...
        if (revealed) {
          rarity = await getTokenRarityCached(tokenId, revealed);
          if (renderId !== latestGridRenderId) return;
        ...
        html += `
          <div class="block-card ${classNames}" data-tokenid="${tokenId}">
            ...
            ${protectionBadge}
            ${farmDurationLine}
            <div class="claim-line-host"><div class="farm-duration" style="opacity:.75;">💰 Checking claim…</div></div>
            ${revealButton}
          </div>
        `;
      }
      ...
      updateGridClaimLinesLazy({
        renderId,
        tokens: subgraphTokens || [],
        v5Map,
        farmMap: state.cachedFarmV6Map
      }).catch((err) => {
        console.error("updateGridClaimLinesLazy error:", err);
      });
    } catch (e) {
  ...
  export async function selectBlock(tokenId, row = null, col = null) {
    ...
    try {
      ...
      resolvedRow = Number(meta?.row ?? resolvedRow ?? 0);
      resolvedCol = Number(meta?.col ?? resolvedCol ?? 0);
    } catch {}
    ...
    const activeOnV5 = await getV5StatusCached(tokenId);
    if (isStaleSelectRequest(requestId)) return;
    ...
    if (farmingActive || activeOnV5) {
      ...
      claimCooldownSeconds = Math.max(0, Number(claim.waitSec || 0));
  
      if (claimReady) {
        claimText = `Ready (${farmingActive ? "V6" : "V5"})`;
      } else if (claimCooldownSeconds > 0) {
        claimText = `${farmingActive ? "V6" : "V5"} in ${formatDuration(claimCooldownSeconds)}`;
      } else {
