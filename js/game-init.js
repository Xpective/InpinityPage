/* =========================================================
  ...
  function applyDashboardViewMode(mode = "simple") {
    const normalized = mode === "commander" ? "commander" : "simple";
    setDashboardViewMode(normalized);
  
    const simpleBtn = byId("simpleViewBtn");
    const commanderBtn = byId("commanderViewBtn");
    const badge = byId("gameViewMode");
  
    if (simpleBtn) {
      simpleBtn.setAttribute("aria-pressed", normalized === "simple" ? "true" : "false");
      simpleBtn.classList.toggle("btn-secondary", normalized !== "simple");
    }
  
    if (commanderBtn) {
      commanderBtn.setAttribute("aria-pressed", normalized === "commander" ? "true" : "false");
      commanderBtn.classList.toggle("btn-secondary", normalized !== "commander");
    }
  
    if (badge) {
      badge.textContent = normalized === "commander" ? "COMMANDER" : "SIMPLE";
    }
  
    document.body.classList.toggle("commander-view", normalized === "commander");
    document.body.classList.toggle("simple-view", normalized !== "commander");
  
    try {
      localStorage.setItem(STORAGE_DASHBOARD_VIEW_KEY, normalized);
    } catch {}
  }
  
  function restoreDashboardViewMode() {
    let stored = "simple";
    try {
      stored = localStorage.getItem(STORAGE_DASHBOARD_VIEW_KEY) || "simple";
    } catch {}
    applyDashboardViewMode(stored);
  }
  ...
  function bindEvents() {
    ...
    /* ==================== VIEW MODE ==================== */
    byId("simpleViewBtn")?.addEventListener("click", () => applyDashboardViewMode("simple"));
    byId("commanderViewBtn")?.addEventListener("click", () => applyDashboardViewMode("commander"));
