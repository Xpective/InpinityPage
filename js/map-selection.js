...
function getOwnedMoveTargetTokenIds() {
  const userLower = state.userAddress?.toLowerCase?.();
  if (!userLower) return [];

  return Object.entries(getAllMapTokens())
    .filter(([tokenId, token]) => {
      if (!token?.owner) return false;
      if (token.owner.toLowerCase() !== userLower) return false;
      return String(tokenId) !== String(mapState.selectedTokenId || "");
    })
    .map(([tokenId]) => String(tokenId))
    .sort((a, b) => Number(a) - Number(b));
}

function bindMoveTargetSelect() {
  const select = byId("moveProtectionTargetTokenId");
  if (!select || select.dataset.bound === "1") return;

  select.addEventListener("change", () => {
    const hasTarget = !!select.value;
    setButtonState(byId("moveProtectionBtn"), hasTarget);
    setButtonState(byId("emergencyMoveProtectionBtn"), hasTarget);
  });

  select.dataset.bound = "1";
}

function updateMoveTargetOptionsForOwner() {
  const select = byId("moveProtectionTargetTokenId");
  if (!select) return;

  bindMoveTargetSelect();

  const options = getOwnedMoveTargetTokenIds();
  const previous = select.value ? String(select.value) : "";

  select.innerHTML = `
    <option value="">Choose target block…</option>
    ${options.map((tokenId) => `<option value="${tokenId}">Block #${tokenId}</option>`).join("")}
  `;
...
function configureMercenaryForOwnerMode({ cleanupAllowed = false } = {}) {
  ...
  showClosest("moveProtectionTargetTokenId", ".detail-row");
  ...
  updateMoveTargetOptionsForOwner();
...
function configureMercenaryForForeignMode({ showPanel = false, cleanupAllowed = false } = {}) {
  ...
  hideClosest("moveProtectionTargetTokenId", ".detail-row");
